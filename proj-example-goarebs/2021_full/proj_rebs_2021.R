#===============================================================================
#  2021 full assessment projections for GOA rougheye/blackspotted rockfish
#===============================================================================
...
stock <- 'goa_rebs'
path <- file.path('2021_full') # changed for demo
species <- 307
group_code <- 'REYE'
endyr <- 2021 # as.numeric(substr(Sys.time(),1,4)) # changed for demo
...
catch_end_date <- as.Date('2021-10-16') # hard coded for the demo
...
out <- tidy_proj(stock = 'goa_rebs',       # stock name (tells the program what .dat file to look for in the assessment model files)
                 curr_yr = 2021,           # the current assessment year (not equal to the fin_mod_yr in a partial assessment year)
