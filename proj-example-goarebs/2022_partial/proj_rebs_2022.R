#===============================================================================
#  2022 partial assessment projections for GOA rougheye/blackspotted rockfish
#===============================================================================
...
stock <- 'goa_rebs'
path <- file.path('2022_partial') # changed for demo
species <- 307
group_code <- 'REYE'
endyr <- 2022 # as.numeric(substr(Sys.time(),1,4)) # assessment year, changed for demo
...
file.copy(from = file.path(paste0('full', endyr-1), 'proj_model'), to = file.path(path), recursive = TRUE)
file.copy(from = file.path(paste0('full', endyr-1), 'final_assessment_model'), to = file.path(path), recursive = TRUE)
...
catch_end_date <- as.Date('2022-10-15') # hard coded for the demo
